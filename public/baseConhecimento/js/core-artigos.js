(function () {
  ...
  async function carregarArtigos() {
    try {
      let url = '/api/conhecimento/artigos?';
      if (state.categoriaAtiva) url += 'categoria_id=' + state.categoriaAtiva + '&';
      const busca = document.getElementById('busca-input').value.trim();
      if (busca) url += 'busca=' + encodeURIComponent(busca) + '&';
      state.artigos = await api.listarArtigos(url);
      renderizarArtigos();
    } catch (erro) {
  ...
  async function salvarArtigo() {
    ...
    try {
      ...
      if (state.artigoAtual && id) abrirArtigo(parseInt(id, 10));
    } catch (erro) {
