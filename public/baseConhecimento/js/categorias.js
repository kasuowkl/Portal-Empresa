(function () {
  ...
  function renderizarCategorias() {
    ...
    for (const cat of state.categorias) {
      const nomeJson = JSON.stringify(cat.nome || '');
      html += `
        <div class="kb-cat-item ${state.categoriaAtiva === cat.id ? 'ativo' : ''}" onclick="filtrarCategoria(${cat.id})" data-cat-id="${cat.id}">
          ...
          <div class="kb-cat-acoes">
            ...
            <button onclick='event.stopPropagation(); excluirCategoria(${cat.id}, ${nomeJson})' title="Excluir"><i class="fas fa-trash"></i></button>
          </div>
        </div>
      `;
    }
  ...
  async function salvarCategoria() {
    ...
    try {
      const resposta = await api.salvarCategoria(id, dados);
  ...
  async function excluirCategoria(id, nome) {
    ...
    try {
      const resposta = await api.excluirCategoria(id);
      if (resposta.erro) return toast(resposta.erro, 'erro');
